...
The normative terminal dependency convention uses contract `host.terminals.v1`, scope ID `<tab_id>/<full lowercase fingerprint_sha256>`, revision `1`, and generation `snapshot.lease_fence.provider_generation`. The helper rejects a noncanonical tab ID, a digest that is not exactly 64 lowercase hexadecimal characters, a scope over the identifier bound, or a zero generation.

The normative state dependency convention uses contract `host.state.v1`, revision `snapshot.revision + 1`, generation `snapshot.store_generation`, and scope ID `state_<digest>`. The lowercase SHA-256 digest covers the exact domain `vqro.host.state.document-dependency.v1\0`, then the store ID and namespace, each encoded as an unsigned 64-bit big-endian UTF-8 byte-length followed by its bytes. Invalid snapshot identity, zero generation, and revision overflow are rejected. Sequence and values are excluded from this compact tuple, so composite audit finalization compares the complete staged, observed, and current `HostStateSnapshotV1` values. The trusted read trace must be exactly one successful `state.snapshot` followed by one successful `terminals.snapshot`; reverse order, missing, duplicate, unrelated, malformed, and failed observations are rejected.

The render request cannot carry either dependency; audit code derives dependencies only from successful trusted host snapshot observations. Production runtime wiring and producer invocation remain follow-up work. The composite state/terminal finalizer is exercised only by synthetic test staging and does not activate the document contract, state reads, rendering, clients, endpoints, or persistence.
